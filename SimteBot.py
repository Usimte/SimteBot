...
term_key = [['Siguiente']]
markupg = ReplyKeyboardMarkup(
    reply_keyboardg, one_time_keyboard=True, selective=True)
markupp = ReplyKeyboardMarkup(reply_keyboardp, one_time_keyboard=True)
markupo = ReplyKeyboardMarkup(
    ok_keyboard, resize_keyboard=True, one_time_keyboard=True)
markupc = ReplyKeyboardMarkup(
    cancel_keyboard, resize_keyboard=True, one_time_keyboard=True)
markupt = ReplyKeyboardMarkup(
    term_key, resize_keyboard=True, one_time_keyboard=True)
...
def addTitle(bot, update, user_data):
    ...
    update.message.reply_text(
        "Escribe una descripción breve de tu tarea máximo 10 palabras", reply_markup=markupc)
    return DC
...
def addDC(bot, update, user_data):
    ...
    if len(text.split(' ')) > 10:
        update.message.reply_text(
            "Su descripción corta excede el máximo de diez palabras, vuelve a escribirla", reply_markup=markupc)
        return DC
    user_data['DescripciónC'] = text
    update.message.reply_text(
        "Escribe la descripción de la tarea, intenta ser claro no te excedas del máximo numero de caracteres de Telegram", reply_markup=markupc)
    return DL
...
def addDL(bot, update, user_data):
    ...
    update.message.reply_text(
        "Escribe cuanto llevas de avance en la tarea usa un número entero entre 0 y 100.Donde 100 significa que la tarea esta completada.", reply_markup=markupc)
    return AVAN
...
def addAvan(bot, update, user_data):
    ...
    if not text.isdigit() or int(text) < 0 or int(text) > 100:
        update.message.reply_text(
            "Escribe un numero entero entre 0 y 100 no agregues espacios.")
        return AVAN
    user_data['avance'] = text
    user = update.message.from_user.username
    user_data['coordinador'] = user
    update.message.reply_text("Vamos a crear una tarea %s" %
                              user_str(user_data), reply_markup=markupo)
    return COOR


def addCoor(bot, update, user_data):
    tareatmp = Tarea(user_data['Titulo'], user_data['DescripciónC'], user_data[
                     'DescripciónL'], user_data['coordinador'], user_data['avance'])
    TAREAS.append(tareatmp)
    saveList(TAREAS)
    update.message.reply_text(
        "se agrego exitosamente le tarea \n %s \ngracias " % tareatmp.show(), reply_markup=markupp)
    user_data.clear()
    return ConversationHandler.END
...
def cancelO(bot, update, user_data):
    update.message.reply_text(
        "Se cancelo la operación, Adiós", reply_markup=markupp)
    user_data.clear()
    return ConversationHandler.END
...
def obtener(bot, update, user_data):
    ...
    if not i.isdigit() or int(i) < 0 or int(i) >= len(TAREAS):
        update.message.reply_text("Escriba un número adecuado entre 0 y menor que  %s" % len(
            TAREAS), reply_markup=markupc)
        return OPCION
...
def modAvan(bot, update, user_data):
    ...
    if not text.isdigit() or int(text) < act or int(text) > 100:
        update.message.reply_text(
            "Escribe un número entero entre %s y 100 no agregues espacios." % str(act))
        return AVAN
    ...
    update.message.reply_text("SI NO desea cambiar cambiar completamente la descripción larga de la tarea oprima la opción Siguiente sino escríbala a continuación. La actual es:\n %s" % user_data[
                              'Tarea'].about, reply_markup=markupt)
    return DONE
...
def modDone(bot, update, user_data):
    ...
    if text == 'Siguiente':
        ...
        user_data['Tarea'].edit(user_data['usuario'], user_data[
                                'avance'], about=text)
    saveList(TAREAS)
    update.message.reply_text("Se modifico la tarea %s" % user_data[
...
def modWork(bot, update, user_data):
    update.message.reply_text(
        "Desea modificar una de las tareas escriba el número de uno de ellas\n %s" % showWorks(), reply_markup=markupc)
...
def verTarea(bot, update, user_data):
    update.message.reply_text("La tarea a la que se quiere %s es:\n %s \n¿esta usted seguro? " % (
        user_data['palabra'], user_data['Tarea'].show()), reply_markup=markupo)
...
def addUser(bot, update, user_data):
    if user_data['Tarea'].add(user_data['usuario']):
        ...
        update.message.reply_text(
            "Usted ya es parte del equipo", reply_markup=markupp)
    user_data.clear()
    return ConversationHandler.END
...
def byePerson(bot, update, user_data):
    update.message.reply_text(
        "Desea retirarse de alguna de las tareas del grupo, escriba el número correspondiente a alguna de ellas \n %s" % showWorks(), reply_markup=markupc)
    user_data['msj'] = "Desea continuar escriba ok de lo contrario cancelar"
...
def showGroup(bot, update, user_data):
    ...
    for x, y in enumerate(mem):
        msj = msj + str(x) + " - @" + y + "\n"
    update.message.reply_text(
        "Seleccione alguno de los miembros del equipo enviando el número correspondiente \n %s" % msj, reply_markup=markupc)
    user_data['Group'] = msj
    return CHOICE
...
def selectU(bot, update, user_data):
    ...
    if not text.isdigit() or int(text) < 0 or int(text) >= len(user_data['Tarea'].group):
        update.message.reply_text(
            "Seleccione una opción valida /n %s" % user_data['Group'], reply_markup=markupc)
        return CHOISE
    user_data['nuevo'] = user_data['Tarea'].group[int(text)]
    update.message.reply_text("Se convertirá el usuario @%s en el coordinador de la tarea %s" % (
        user_data['nuevo'], user_data['Tarea'].title), reply_markup=markupo)
    return DONE
...
def coorUser(bot, update, user_data):
    if user_data['Tarea'].delegate(update.message.from_user.username, user_data['nuevo']):
        ...
        update.message.reply_text(
            "Usted no es el coordinador y solo el puede hacer esta operación.", reply_markup=markupp)
    user_data.clear()
    return ConversationHandler.END
...
def passCoor(bot, update, user_data):
    update.message.reply_text(
        "Desea delegar la coordinación a otra persona,asegúrese que la persona haga parte del equipo de trabajo de la tarea y escriba el número correspondiente a la tarea que desea\n %s" % showWorks(), reply_markup=markupc)
...
def salir(bot, update, user_data):
    update.message.reply_text(
        "Gracias Adiós", reply_markup=ReplyKeyboardHide())
    print str(user_data)
    user_data.clear()
    return ConversationHandler.END
...
def main(TOKEN):
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler('start' + PASS, start)],
        states={
            CHOOSING: [RegexHandler('^ver tareas$',
                ...
                fallbacks=[CommandHandler(
                    'Cancelar', cancelO, pass_user_data=True)]
            ),
                ConversationHandler(
